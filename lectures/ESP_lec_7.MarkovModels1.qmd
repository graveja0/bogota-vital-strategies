...
::: incremental
-   Discutir los pros y los contras de la modelización de decisiones
...
::: incremental
-   Supongamos que queremos modelizar la relación coste-eficacia de
    estrategias estrategias alternativas para prevenir la aparición de
    una enfermedad.
...
## Un Simple Proceso de Enfermedad

::: incremental
-   Permanecer sano no conlleva disminución de utilidad (peso de
    utilidad = 1.0 por ciclo en estado sano)
-   Enfermar conlleva una disminución de utilidad de 0,25 durante el
    resto de la vida de la persona (peso de utilidad = 0,75). vida de la
    persona (peso de la utilidad = 0,75)
...
::: incremental
-   Permanecer sano no tiene ningún coste.
-   Enfermar supone un coste de 1.000 \$ al año.
...
preventiva que reducen el riesgo de enfermar:

| Estrategia | Descripción                                       | Coste        |
|------------------|------------------------------------|------------------|
...
-   Una opción sería utilizar un árbol de decisión para modelar la
    utilidad esperada y los costes asociados a cada estrategia. utilidad
    esperada y los costes asociados a cada estrategia.
...
-   Idealmente, queremos un enfoque de modelado que pueda incorporar
    ...
    de decisión difíciles de manejar.
...
-   Como hay un número fijo de estados de enfermedad en los que puede
    encontrarse la población la población, no es necesario modelar vías
    complejas, como vimos en la como vimos en la "explosión" del árbol
    de decisión hace unas diapositivas.
:::
...
-   Los individuos sólo pueden existir en un estado a la vez (estados de
    salud mutuamente estados de salud mutuamente excluyentes)
-   Al principio o al final de cada ciclo, los pacientes pasan por
    ...
    individuos permanecen en estado de salud [durante todo el ciclo
    length]{style="color de fondo: amarillo;"}
...
Ya hemos definido el problema de decisión anteriormente en esta clase,
...
### {{< fa check-circle >}} 3b. Curar y definir las entradas del modelo
...
-   Una consecuencia es que el modelo nos mostrará qué fracción empieza
    ...
    estado al final del ciclo. al final del ciclo.
:::
...
-   Riesgos concurrentes: los individuos pueden pasar de su estado de
    salud estado de salud actual a otros dos o más estados de salud.
:::
:::
...
los **riesgos en competencia**.

::: columns
::: {.column width="50%"}
::: incremental
-   Si no tenemos cuidado, podríamos descartar la posibilidad... de la
    Salud {{< fa arrow-right-long >}} Enfermo{{< fa arrow-right-long >}}
    Muerto dentro de un ciclo.
-   El modelo sería *como* un sano básico
    {{< fa flecha-derecha-larga >}} Muerto, pero se desvió a través de
...
-   Los modelos de Markov carecen de memoria: no recuerdan lo que
    ...
    -   Si su riesgo de transición a un estado de salud más grave
        ...
        tenerlo en cuenta explícitamente. explícitamente.
...
-   Otra opción es incorporar los acontecimientos "a corto plazo" que se
    ...
    de decisión y de decisión y, a continuación, permitir que el modelo
    de Markov modele las consecuencias sanitarias a largo plazo.
:::
:::
...
::: incremental
-   Tasa de aparición de la enfermedad
-   Utilidades y costes del estado de salud
-   Ratios de riesgo, odds ratios o riesgos relativos para diferentes
...
Hemos definido muchos de los parámetros subyacentes antes en esta
conferencia, por lo que los repetiremos brevemente aquí.
...
-   Permanecer sano no conlleva ninguna disminución de la utilidad (peso
    de la utilidad= 1,0)
-   Enfermar conlleva una disminución de la utilidad de 0,25 durante el
    resto de la vida de la persona (peso de la utilidad= 1,0). vida de
    la persona (peso de utilidad = 0,75)
-   La muerte tiene un peso de utilidad de 0.
:::

## 3. Parametrizar el modelo

-   No hay ningún coste asociado a mantenerse sano.

::: incremental
-   Enfermar supone un coste de 1.000 \$/año.
-   Enfermar aumenta el riesgo de muerte en un 300%.
:::

## 3. Parametrizar el modelo

Cada estrategia tiene un coste y un impacto diferentes en la
probabilidad de enfermar. enfermar.
...
-   A menudo, los parámetros se extraen de la literatura publicada, y es
    ...
    -   Por ejemplo, el parámetro de reducción porcentual del riesgo de
        ...
        estrategia puede proceder de diferentes ensayos clínicos.
...
-   Algunos parámetros pueden ser simplemente valores (por ejemplo, el
    coste de la estrategia A es de \\25 \$/año)
-   Algunos parámetros pueden ser funciones de otros parámetros.
    ...
    -   Podemos utilizar estos dos parámetros para deducir el número
        total de ciclos que necesitamos realizar. ciclos que necesitamos
        ejecutar.
:::
...
-   Todo lo anterior resalta la importancia de adoptar un proceso formal
    ...
    incertidumbre de de **todos** los parámetros del modelo en un único
    lugar.

-   Recomendamos un enfoque estructurado basado en convenciones de
    nomenclatura de parámetros y tablas de parámetros. y tablas de
    parámetros.
:::
...
knitr::kable(
  dplyr::tibble(
    type = c("Probabilidad", "Tasa (Rate)", "Matriz", "Coste", "Utilidad", "Razón de riesgo (hazard ratio)"),
...
que trataremos en nuestro taller intermedio (de una semana de duración).
cubrimos en nuestro taller intermedio (de una semana de duración).
...
-   La matriz de probabilidad de transición es una matriz cuadrada que
    ...
    estado de salud en un solo paso de tiempo.

-   Construir la matriz es un proceso bastante técnico, pero bastante
    pero bastante sencillo.

    -   Podríamos dedicarle un día entero de clases y casos prácticos (y
        lo hacemos en nuestra semana). (¡y lo hacemos en nuestro curso
        intermedio de una semana!)
...
::: r-vstack
::: {data-id="occ"}
Estado de salud Ocupación al inicio del ciclo
:::
...
::: r-hstack
::: {data-id="occ"}
Ocupación del estado de salud al inicio del ciclo
:::
...
::: {data-id="occ1"}
Estado de salud Ocupación al final del ciclo
:::
:::
...
## Trazado de Markov {auto-animate="true" auto-animate-easing="ease-in-out"}

::: {data-id="occ1"}
Estado de salud Ocupación durante diez ciclos
:::
...
-   Pregunta de investigación: ¿Es el nuevo fármaco rentable en
    comparación con la comparado con el tratamiento estándar actual?
...
## Un ejemplo de Trazado de Markov (horizonte de 75 años)
...
- Por lo tanto, cada estrategia tendrá su propia traza de Markov.

## Cálculo de resultados

- Con nuestras trazas de Markov completas, podemos calcular los resultados esperados
    (por ejemplo, costes, AVAC, AVAD, etc.).
- Al igual que hicimos con los árboles de decisión, tenemos que definir los "pagos" para cada estado de salud.
    para cada estado de salud.

## Definición de los resultados

- Coste de los resultados: Coste de estar sano, enfermo, muerto (incluido cualquier
    costes adicionales del tratamiento/intervención, si procede).
- Resultados QALY: Peso de la utilidad de estar sano (normalmente 1,0), enfermo
    muerto (normalmente 0,0).
- Resultados AVAD: Pesos de discapacidad de estar enfermo (YLD) y esperanza de vida
    esperanza de vida restante basada en la tabla de vida de referencia (YLL)

## Definición de las compensaciones

::: nonincremental
- Podemos calcular la retribución total de cada ciclo multiplicando el
    multiplicando el número o la fracción de la cohorte en cada estado de salud por su valor de compensación y sumándolos.
    y sumándolos.
:::

Ejemplo de traza de Markov (dos ciclos):
...
- Construiremos nuestro ejemplo utilizando un resultado simple: años de vida esperados
    bajo una estrategia dada.
- Resultados:
    -   Healthy: [1.0]{style="color:green;"}
    -   Sick: [1.0]{style="color:green;"} \[they're still alive!\]
    -   Dead: [0.0]{style="color:green;"}

## Ejemplo: Años de vida

- Para obtener la "retribución" total de un ciclo determinado, multiplicamos la
    fracción de la cohorte en un estado de salud dado por la retribución
    asociado a ese estado de salud.
...
-   Payoffs:
    -   Healthy: [\$0]{style="color:blue;"}
    -   Sick: [\$1,000]{style="color:blue;"}
    -   Dead: [\$0]{style="color:blue;"}

## Ejemplo: Costes

¿Cuál es el coste del ciclo 0?
...
¿Cuál es el coste "compensatorio" del Ciclo 1?
...
- Podemos repetir un proceso similar para los resultados sanitarios (por ejemplo, AVAC,
    YLD) multiplicando el "resultado" (por ejemplo, el peso de la utilidad, el peso de la discapacidad) para un estado de salud determinado por la fracción de la cohorte que se encuentra en un estado de salud determinado.
    discapacidad) de un determinado estado de salud por la fracción de la cohorte que se encuentra en ese estado de salud en el ciclo.
    ese estado de salud en el ciclo.
...
Veamos la traza de Markov y los resultados del ciclo para el resultado Años de vida
resultado.
...
-   The cumulative LYs for an individual starting in the Healthy state
    is **44.825**
...
- Podemos hacer un ejercicio similar para obtener los costes totales, los AVAC totales, los AVAD totales, etc.
    AVAD, etc.
...
- En la vida real, los eventos podrían ocurrir en cualquier punto de un ciclo dado, pero
    un modelo de Markov asume que todos los eventos ocurren al principio o al
    final de cada ciclo

- El tiempo es continuo, también lo son las curvas de supervivencia/supervivencia sin eventos

- Cuando discretizamos el tiempo utilizando una longitud de ciclo fija, podemos hacer
    dos supuestos

    - Supongamos que se trata de un simple bien $\arrow$ proceso muerto
...
::: fragment
Sobreestima la pertenencia a un estado en Well
:::
:::
...
::: fragment
Subestima la pertenencia a un estado en Well
:::
:::
:::
...
- Desplazar la curva calculada de pertenencia a un estado discreto hacia la izquierda
    en 1/2 ciclo.
...
- Una vez que tenemos los resultados totales (descontados, corregidos por medio ciclo) para
    cada estrategia, podemos pasar a realizar un análisis
