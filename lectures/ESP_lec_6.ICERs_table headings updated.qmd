...
-   Es una medida explícita de la relación valor-precio
...
## Relación costo-efectividad {auto-animate="true" auto-animate-easing="ease-in-out"}
...
## Relación costo-efectividad incremental {.smaller}
...
## Relación costo-efectividad promedia {.smaller}
...
RCEI &= \frac{C_1 - 0}{E_1 - 0} \\\frac{C_1}{E_1 - 0}
 &= \frac{C_1}{E_1 }
\fin{alineado}
$$
:::
...
-   Cada programa se compara con una alternativa nula. Calcular una
    relación costo-efectividad "media".
...
-   Es necesario calcular una "relación costo-efectividad incremental",
...
::: incremental
Paso 3:\
- Clasificar otros programas en orden ascendente según su relación
...
::: {.column width="50%"}
::: incremental
-   Calcular la relación costo-efectividad promedia.
:::
:::
...
::: notes
-   Queremos ordenar los RELACIONES de menor a mayor para la inclusión
...
#### {{< fa circle-check >}} La relación costo-efectividad incremental está por debajo de un umbral de adopción preespecificado.
...
3.  Requiere el cálculo de una *Relación costo-efectividad promedia*.
:::
...
2.  Diferentes costos y efectos asociados a cada uno.
3.  [Requiere el cálculo de una *Relación Costo-Efectividad Incremental*
...
-   A continuación se calculan los costos y efectos de cada estrategia
    en relación con el respecto al statu quo.
...
-   La pendiente de una línea que une dos puntos es la relación
...
(2) Supongamos que comparamos la vacunación antigripal con la no
    vacunación & que nuestra relación costo-efectividad arroja un precio
...
Notas (1) Hay diferentes formas de determinar un umbral rentable & se
...
::: notas
(1) Otros umbrales que han utilizado los países se basan en el PIB per
    cápita. PIB per cápita; principalmente el umbral de 1-3X PIB per
    cápita

(2) Aunque este intervalo de umbrales se corresponde aproximadamente con
    lo que se ha convertido en convención para los países de renta alta
    (1X PIB per cápita en EE.UU. es 60.000 dólares y 3 veces ese valor
    es el límite superior que utilizamos. 50.000 como límite inferior,
    100.000 como base y 150.000 como límite superior). como extremo
    superior)

(3) PERO el consumo per cápita en los países más ricos supera el consumo
    per cápita en los países de renta baja y media. per cápita de los
    países de renta baja y media en uno o dos órdenes de magnitud.
    órdenes de magnitud y, por tanto, algunos analistas sostienen que el
    gasto sanitario debería representar una parte menor del PIB per
    cápita per cápita en los países de renta baja y media.
:::
...
::: notas
(1) Los analistas también han argumentado que el umbral debería ser más
    bajo para estos países basándose en una perspectiva de presupuesto
    sanitario

(2) En Perú, por ejemplo, un estudio halló que la adición de un
    tratamiento del cáncer de mama de mama podría ser rentable según el
    criterio general de 1 a 3 veces el PIB per cápita. per cápita; sin
    embargo, el costo de añadir el nuevo fármaco superaría el
    presupuesto total de Perú para el cáncer de mama. superaría todo el
    presupuesto de Perú para el tratamiento del cáncer de mama.
:::
...
-   La proporción de estudios PUBLICADOS que citan un umbral de 1 a 3
    veces el PIB per cápita ha aumentado con el tiempo. per cápita ha
    aumentado con el tiempo y esto se corresponde en gran medida con el
    reconocimiento de este umbral por parte de la OMS a principios de la
    década de 2000. Antes de esto, la mayoría de los estudios no citaban
    ningún umbral u otros umbrales de EC.
...
(3) Hasta que encontremos una medida mejor, el DSP3 y otros académicos
    han estimado umbrales más conservadores (diapositiva anterior) que
    yo para los PIBM y NO utilizaría los supuestos per cápita.
