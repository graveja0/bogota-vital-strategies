...
Este caso práctico está diseñado para que se familiarice con la construcción de un árbol de decisión en Amua. Los objetivos específicos de aprendizaje son los siguientes:

1.  Parametrizar y estructurar un modelo de árbol de decisión.

2.  

    2.  Estimar los resultados de supervivencia y coste a partir del modelo de árbol de decisión en el software Amua.
...
En este problema de ejemplo, debe decidir si implantar o no un programa de cribado para una determinada enfermedad (Dx). Suponga que las personas que den positivo serán tratadas, lo que mejorará su esperanza de vida. En el caso de los individuos positivos a la enfermedad que no se someten a cribado, suponga que el tratamiento se retrasará hasta que presenten síntomas y que tienen una esperanza de vida inferior a los que reciben tratamiento inmediato.

En el caso de los individuos negativos a la enfermedad que reciben un tratamiento inadecuado, supongamos que su esperanza de vida disminuye debido a la toxicidad de los fármacos. La prueba de detección es imperfecta, con una sensibilidad de 0,97 y una especificidad de 0,9988. La prevalencia de la enfermedad en esta población es de 0,05. El objetivo es maximizar la esperanza de vida. Como estrategias alternativas, se podría no cribar a nadie, o simplemente tratar a todos sin cribar primero.
...
-   Haga clic con el botón derecho: Haga clic con el botón derecho en el nodo para mostrar el menú emergente. Seleccione Añadir y elija el tipo de nodo que desea añadir. que desee añadir.
...
Adapte la estrategia Cribar para incluir un nodo de azar que indique la probabilidad de un resultado positivo.
...
Ahora puede completar la estructura del árbol de decisión añadiendo los nodos terminales restantes para la estrategia Tratar todo.

![](imágenes/amua-terminal3.png)
...
Por defecto, los resultados del modelo (denominados "dimensiones" del modelo en Amua) hacen un seguimiento de los Costes, denotados por el símbolo \$ dentro de un paréntesis. entre paréntesis. En este modelo, nuestro resultado de interés será la Esperanza de Vida.
...
-   En esta pestaña, cambie el nombre de la dimensión de **Coste** a **Esperanza de Vida**, cambie el símbolo a LE, y elija 2 decimales para la precisión.
...
## Definir probabilidades para la estrategia \`Screen
...
-   En la estrategia "No Screen" para **D +**, introduzca "LateRx" como recompensa, que definiremos como la esperanza de vida de las personas que reciben tratamiento tarde.

    -   Para **D -**, introduzca `NoDx` como resultado, que será la esperanza de vida de las personas sin la enfermedad.

-   En la estrategia Screen, sólo se tratará a las personas que den positivo.

    -   En la rama **Prueba +**, introduzca `Rx` como resultado para **D +** para indicar que estas personas serán tratadas.
    -   Para **D-**, introduzca `Tox` como recompensa, ya que estas personas son falsos positivos y se ven afectadas negativamente por la toxicidad del tratamiento.
    -   En la rama **Test-**, introduzca `LateRx` como recompensa para **D +**, ya que estas personas tienen la enfermedad pero no fueron identificadas por la prueba de cribado.
    -   Para **D -**, introduzca `NoDx` ya que estas personas son verdaderos negativos.

-   Todos serán tratados en la estrategia "Tratar a todos". Esto es bueno para los que tienen la enfermedad, pero no para los que no la tienen. sin la enfermedad.
...
-   Cuando esté listo para ejecutar el modelo, puede hacer clic en <i class="fa-solid fa-circle-play" style="color: #18af59;"></i> Ejecutar modelo, que evaluará el modelo y mostrará los resultados en el lienzo y en la consola.
